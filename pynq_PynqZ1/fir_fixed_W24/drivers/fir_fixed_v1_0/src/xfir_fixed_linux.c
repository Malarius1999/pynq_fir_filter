...
typedef struct {
    ...
    XFir_fixed_uio_map maps[ MAX_UIO_MAPS ];
} XFir_fixed_uio_info;

/***************** Variable Definitions **************************************/
static XFir_fixed_uio_info uio_info;
...
static int uio_info_read_name(XFir_fixed_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XFir_fixed_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XFir_fixed_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XFir_fixed_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XFir_fixed_Initialize(XFir_fixed *InstancePtr, const char* InstanceName) {
	XFir_fixed_uio_info *InfoPtr = &uio_info;
